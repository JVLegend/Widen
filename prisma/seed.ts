...
async function main() {
  ...
  const [church1, church2, church3, church4, church5, grace] = await Promise.all([
    ...
    prisma.user.create({ data: { email: "grace@example.com", password: "123456", name: "Grace Community Church", avatarUrl: "https://api.dicebear.com/9.x/avataaars/svg?seed=grace", role: "influencer", bio: "Growing church passionate about spreading God's Word through digital media. Pastor James Wilson." } }),
  ]);

  // ─── Creators (role: "clipper" in DB) ─────────────────────────────────────
  const [cr1, cr2, cr3, cr4, cr5, cr6, cr7, cr8, cr9, cr10, sarah] = await Promise.all([
    ...
    prisma.user.create({ data: { email: "sarah.chen@example.com", password: "123456", name: "Sarah Chen",     avatarUrl: "https://api.dicebear.com/9.x/avataaars/svg?seed=sarahchen", role: "clipper", bio: "Creative editor. Specialist in viral sermon shorts for TikTok and Reels. Top 5 monthly ranking." } }),
  ]);
  ...
  await prisma.socialAccount.createMany({
    data: [
      ...
      { userId: grace.id, platform: "youtube",   handle: "@gracecommunitych",   profileUrl: "https://youtube.com/@gracecommunitych",   followers: 245000 },
      { userId: grace.id, platform: "instagram", handle: "@grace.community",    profileUrl: "https://instagram.com/grace.community",    followers: 198000 },
      { userId: grace.id, platform: "tiktok",    handle: "@gracecommunity",     profileUrl: "https://tiktok.com/@gracecommunity",     followers: 312000 },
    ],
  });
  ...
  const acc = await Promise.all([
    ...
    /* 21 */ prisma.socialAccount.create({ data: { userId: sarah.id, platform: "tiktok",    handle: "@sarah.chen",       profileUrl: "https://tiktok.com/@sarah.chen",       followers: 185000 } }),
    /* 22 */ prisma.socialAccount.create({ data: { userId: sarah.id, platform: "instagram", handle: "@sarahchencreates", profileUrl: "https://instagram.com/sarahchencreates", followers: 92000 } }),
    /* 23 */ prisma.socialAccount.create({ data: { userId: sarah.id, platform: "youtube",   handle: "@sarahchen",        profileUrl: "https://youtube.com/@sarahchen",        followers: 47000  } }),
  ]);
  ...
  const [
    ...
    // Grace Community
    prisma.video.create({ data: { influencerId: grace.id, title: "The Gospel of Grace — Ephesians Series", description: "Complete teaching series on the book of Ephesians.", originalUrl: "https://youtube.com/watch?v=ephesians", platform: "youtube", thumbnailUrl: "https://picsum.photos/seed/g1/640/360", tags: JSON.stringify(["grace", "ephesians", "gospel"]), status: "in_campaign" } }),
    prisma.video.create({ data: { influencerId: grace.id, title: "Faith Over Fear — Conquering Anxiety", description: "Powerful message about overcoming anxiety through faith.", originalUrl: "https://youtube.com/watch?v=faith-over-fear", platform: "youtube", thumbnailUrl: "https://picsum.photos/seed/g2/640/360", tags: JSON.stringify(["faith", "anxiety", "mental health"]), status: "in_campaign" } }),
    prisma.video.create({ data: { influencerId: grace.id, title: "Generosity: The Heart of Giving", description: "Teaching on biblical generosity and stewardship.", originalUrl: "https://youtube.com/watch?v=generosity", platform: "youtube", thumbnailUrl: "https://picsum.photos/seed/g3/640/360", tags: JSON.stringify(["generosity", "stewardship", "giving"]), status: "in_campaign" } }),
    prisma.video.create({ data: { influencerId: grace.id, title: "Discipleship 101 — Following Jesus Daily", description: "Practical steps for daily discipleship and spiritual growth.", originalUrl: "https://youtube.com/watch?v=discipleship", platform: "youtube", thumbnailUrl: "https://picsum.photos/seed/g4/640/360", tags: JSON.stringify(["discipleship", "growth", "daily"]), status: "in_campaign" } }),
    prisma.video.create({ data: { influencerId: grace.id, title: "The Great Commission — Go and Make Disciples", description: "Call to action for evangelism and missions.", originalUrl: "https://youtube.com/watch?v=great-commission", platform: "youtube", thumbnailUrl: "https://picsum.photos/seed/g5/640/360", tags: JSON.stringify(["evangelism", "missions", "commission"]), status: "in_campaign" } }),
    prisma.video.create({ data: { influencerId: grace.id, title: "Praise Night — Live Worship Experience", description: "Full evening of worship, prayer, and praise.", originalUrl: "https://youtube.com/watch?v=praise-night", platform: "youtube", thumbnailUrl: "https://picsum.photos/seed/g6/640/360", tags: JSON.stringify(["worship", "praise", "live"]), status: "available" } }),
  ]);

  // ─── Missions (Campaigns) ────────────────────────────────────────────────
  const [miss1, miss2, miss3, miss4, miss5, miss6, miss7,
         missG1, missG2, missG3] = await Promise.all([
    ...
    // ── Grace Community — 3 missions ────────────────────────────────────────
    prisma.campaign.create({ data: {
      influencerId: grace.id,
  ...
  const clips: ClipData[] = [
    ...
    // ══ Sarah Chen — content across multiple missions ═══════════════════
    { clipperId: sarah.id, campaignId: miss1.id, videoId: v1.id, socialAccountId: acc[21].id, clipUrl: "https://tiktok.com/@sarah.chen/v/faith1",       platform: "tiktok",    status: "active",           views: 470000,  likes: 33000, comments: 2600, shares: 11000, earnings: 11750, publishedAt: daysAgo(21) },
    { clipperId: sarah.id, campaignId: miss1.id, videoId: v1.id, socialAccountId: acc[22].id, clipUrl: "https://instagram.com/reel/sarah-faith1",       platform: "instagram", status: "active",           views: 156000,  likes: 11200, comments: 840,  shares: 2800,  earnings: 3900,  publishedAt: daysAgo(17) },
    { clipperId: sarah.id, campaignId: miss3.id, videoId: v4.id, socialAccountId: acc[21].id, clipUrl: "https://tiktok.com/@sarah.chen/v/youth1",       platform: "tiktok",    status: "metrics_collected", views: 310000, likes: 22000, comments: 1700, shares: 7200,  earnings: 60,    publishedAt: daysAgo(14) },
    { clipperId: sarah.id, campaignId: miss3.id, videoId: v6.id, socialAccountId: acc[22].id, clipUrl: "https://instagram.com/reel/sarah-youth2",       platform: "instagram", status: "active",           views: 89000,   likes: 6400,  comments: 490,  shares: 1500,  earnings: 60,    publishedAt: daysAgo(11) },
    { clipperId: sarah.id, campaignId: miss5.id, videoId: v7.id, socialAccountId: acc[21].id, clipUrl: "https://tiktok.com/@sarah.chen/v/easter1",      platform: "tiktok",    status: "active",           views: 680000,  likes: 49000, comments: 3800, shares: 16000, earnings: 20400, publishedAt: daysAgo(16) },
    { clipperId: sarah.id, campaignId: miss6.id, videoId: v9.id, socialAccountId: acc[22].id, clipUrl: "https://instagram.com/reel/sarah-psalm1",       platform: "instagram", status: "active",           views: 245000,  likes: 19000, comments: 1400, shares: 5800,  earnings: 5390,  publishedAt: daysAgo(19) },
    { clipperId: sarah.id, campaignId: miss6.id, videoId: v10.id, socialAccountId: acc[21].id, clipUrl: "https://tiktok.com/@sarah.chen/v/fam1",        platform: "tiktok",    status: "active",           views: 520000,  likes: 38000, comments: 2900, shares: 13000, earnings: 11440, publishedAt: daysAgo(12) },
    { clipperId: sarah.id, campaignId: miss7.id, videoId: v11.id, socialAccountId: acc[21].id, clipUrl: "https://tiktok.com/@sarah.chen/v/pray1",       platform: "tiktok",    status: "active",           views: 730000,  likes: 54000, comments: 4200, shares: 18000, earnings: 10950, publishedAt: daysAgo(9)  },
    { clipperId: sarah.id, campaignId: miss7.id, videoId: v11.id, socialAccountId: acc[23].id, clipUrl: "https://youtube.com/@sarahchen/shorts/pray1",   platform: "youtube",   status: "active",           views: 125000,  likes: 8900,  comments: 680,  shares: 2400,  earnings: 1875,  publishedAt: daysAgo(6)  },
    { clipperId: sarah.id, campaignId: missG2.id, videoId: vg3.id, socialAccountId: acc[21].id, clipUrl: "https://tiktok.com/@sarah.chen/g1",           platform: "tiktok",    status: "active",           views: 395000,  likes: 28000, comments: 2100, shares: 8500,  earnings: 11060, publishedAt: daysAgo(19) },
    { clipperId: sarah.id, campaignId: missG2.id, videoId: vg4.id, socialAccountId: acc[22].id, clipUrl: "https://instagram.com/reel/sarah-grace2",     platform: "instagram", status: "active",           views: 210000,  likes: 15200, comments: 1150, shares: 4200,  earnings: 5880,  publishedAt: daysAgo(8)  },
    { clipperId: sarah.id, campaignId: missG2.id, videoId: vg5.id, socialAccountId: acc[23].id, clipUrl: "https://youtube.com/@sarahchen/shorts/grace3", platform: "youtube",   status: "active",           views: 88000,   likes: 6300,  comments: 480,  shares: 1700,  earnings: 2464,  publishedAt: daysAgo(3)  },
  ];

  await Promise.all(clips.map((c) => prisma.clip.create({ data: c })));

  // ─── Rankings ─────────────────────────────────────────────────────────────
  // Sarah total: 470K+156K+310K+89K+680K+245K+520K+730K+125K+395K+210K+88K = 4,018,000 views, 12 clips
  const rankingData = [
    { clipperId: sarah.id, totalViews: 4018000, totalClips: 12, rankPosition: 1,  badge: "top1"  },
  ...
  console.log(`  6 churches (incl. Grace Community)`);
  console.log(`  11 creators (incl. Sarah Chen)`);
  console.log(`  18 sermons (6 from Grace Community)`);
  console.log(`  10 missions (3 from Grace Community: completed, active, draft)`);
  console.log(`  ${clips.length} content pieces (12 from Sarah Chen)`);
  console.log(`  ${rankingData.length * 2} ranking entries`);
}
